...
public final class MetalFormerMenu extends AbstractContainerMenu {
    ...
    public MetalFormerMenu(int containerId, Inventory playerInventory, FriendlyByteBuf buffer) {
        this(containerId, playerInventory, (MetalFormerBlockEntity) playerInventory.player.level().getBlockEntity(buffer.readBlockPos()), null);
    }

    public MetalFormerMenu(int containerId, Inventory playerInventory, MetalFormerBlockEntity blockEntity, ContainerData data) {
    ...
    public boolean stillValid(Player player) {
        return stillValid(ContainerLevelAccess.create(blockEntity.getLevel(), blockEntity.getBlockPos()), player, IC2Blocks.METAL_FORMER.get());
    }
