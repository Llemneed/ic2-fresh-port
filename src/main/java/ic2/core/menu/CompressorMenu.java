...
public final class CompressorMenu extends AbstractContainerMenu {
    ...
    public CompressorMenu(int containerId, Inventory playerInventory, FriendlyByteBuf buffer) {
        this(containerId, playerInventory, (CompressorBlockEntity) playerInventory.player.level().getBlockEntity(buffer.readBlockPos()), null);
    }

    public CompressorMenu(int containerId, Inventory playerInventory, CompressorBlockEntity blockEntity, ContainerData data) {
    ...
    public boolean stillValid(Player player) {
        return stillValid(ContainerLevelAccess.create(blockEntity.getLevel(), blockEntity.getBlockPos()), player, IC2Blocks.COMPRESSOR.get());
    }
