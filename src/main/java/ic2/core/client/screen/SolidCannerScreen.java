...
public final class SolidCannerScreen extends AbstractIc2MachineScreen<SolidCannerMenu> {
    private static final List<Component> RECIPE_HINTS = List.of(
            Component.literal("Bread + Tin Can -> Filled Tin Can"),
            Component.literal("Apple + Tin Can -> Filled Tin Can"),
            Component.literal("Cooked food + Tin Can -> Filled Tin Can")
    );
    private boolean showRecipeHints;

    public SolidCannerScreen(SolidCannerMenu menu, Inventory playerInventory, Component title) {
    ...
    protected void renderBg(GuiGraphics guiGraphics, float partialTick, int mouseX, int mouseY) {
        ...
        drawSlot(guiGraphics, left + 36, top + 35);
        drawSlot(guiGraphics, left + 66, top + 35);
        drawSlot(guiGraphics, left + 115, top + 35);
        drawSlot(guiGraphics, left + 7, top + 61);
        drawRecipeButton(guiGraphics, left + 88, top + 35);
        drawDecorativeUpgradeColumn(guiGraphics, left + 151, top + 7);
        drawPlayerInventory(guiGraphics, left, top);
        drawEnergyBar(guiGraphics, left + 11, top + 46, menu.getScaledEnergy());
        drawCannerArrow(guiGraphics, left + 54, top + 35);
        drawProgress(guiGraphics, left + 89, top + 36, menu.getScaledProgress());
    }
    ...
    public boolean mouseClicked(double mouseX, double mouseY, int button) {
        ...
        if (localX >= 88 && localX < 110 && localY >= 35 && localY < 50 && button == 0) {
    ...
    private void drawCannerArrow(GuiGraphics guiGraphics, int x, int y) {
        guiGraphics.fill(x, y, x + 12, y + 18, 0x00000000);
        guiGraphics.fill(x + 4, y, x + 8, y + 18, 0xFF808080);
        guiGraphics.fill(x + 2, y + 6, x + 10, y + 12, 0xFFC0C0C0);
    }

    private void drawProgress(GuiGraphics guiGraphics, int x, int y, int progress) {
        int fill = Math.max(0, Math.min(13, progress));
        guiGraphics.fill(x + 6, y + 7, x + 19, y + 9, 0xFF404040);
        if (fill > 0) {
            guiGraphics.fill(x + 6, y + 7, x + 6 + fill, y + 9, 0xFFF0F0F0);
        }
    }
    ...
    private void renderRecipeHints(GuiGraphics guiGraphics) {
        ...
        guiGraphics.drawString(font, "Solid canner recipes", panelX + 6, panelY + 6, 0xFF55FF55, false);
    ...
    private void renderContextTooltips(GuiGraphics guiGraphics, int mouseX, int mouseY) {
        ...
        if (isInside(localX, localY, 88, 35, 22, 15)) {
            guiGraphics.renderTooltip(font, tooltipLines(
                    Component.literal("Recipe help"),
                    Component.literal("Shows solid canner recipes")
            ), mouseX, mouseY);
        }
    }
