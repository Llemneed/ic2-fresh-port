...
public final class ExtractorScreen extends AbstractIc2MachineScreen<ExtractorMenu> {
    private static final List<Component> RECIPE_HINTS = List.of(
            Component.literal("Sticky Resin -> 3 Rubber"),
            Component.literal("Rubber Wood -> Rubber"),
            Component.literal("Rubber Sapling -> Rubber"),
            Component.literal("Clay Block -> 4 Clay Balls"),
            Component.literal("Bricks -> 4 Bricks"),
            Component.literal("Snow Block -> 4 Snowballs")
    );
    ...
    protected void renderBg(GuiGraphics guiGraphics, float partialTick, int mouseX, int mouseY) {
        ...
        drawExtractorProgress(guiGraphics, left + 80, top + 35, menu.getScaledProgress());
    }
    ...
    private void drawExtractorProgress(GuiGraphics guiGraphics, int x, int y, int progress) {
        guiGraphics.fill(x + 6, y + 2, x + 8, y + 12, 0xFFEAEAEA);
        int fill = Math.max(0, Math.min(13, progress));
        if (fill > 0) {
            guiGraphics.fill(x + 9, y + 7, x + 9 + fill, y + 9, 0xFFF0F0F0);
            guiGraphics.fill(x + 8 + fill, y + 5, x + 11 + fill, y + 11, 0xFFF0F0F0);
        }
    }
    ...
    private void renderRecipeHints(GuiGraphics guiGraphics) {
        ...
        guiGraphics.drawString(font, "Extractor recipes", panelX + 6, panelY + 6, 0xFF55FF55, false);
    ...
    private void renderContextTooltips(GuiGraphics guiGraphics, int mouseX, int mouseY) {
        ...
        if (isInside(localX, localY, 80, 35, 22, 15)) {
            guiGraphics.renderTooltip(font, tooltipLines(
                    ...
                    Component.literal("Shows extractor recipes")
            ), mouseX, mouseY);
            return;
        }

        if (isInside(localX, localY, 151, 7, 18, 72)) {
            guiGraphics.renderTooltip(font, tooltipLines(
                    Component.literal("Upgrade slots"),
                    Component.literal("Uses the same upgrades as the macerator")
            ), mouseX, mouseY);
        }
    }
